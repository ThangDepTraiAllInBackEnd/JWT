...
namespace CleanArchitecture.Controllers
{
	...
	public class EmployeeController : ControllerBase
	{
		...
		///  created by: Nguyễn Thiện Thắng
		///  created at: 2023/12/2
		[HttpDelete("/api/v1/Employee/ManyDelete")]
		public async Task<IActionResult> Deletemany([FromBody] List<Employee> employees)
		{
			ServiceResult res = await _employeeService.DeleteManyAsync(employees);
			return StatusCode((int)res.Code, res);
		}
		 ...
		 [FromQuery(Name = "key")] string? key = null)
		{
			...
			ServiceResult res = await _employeeService.PagingServiceAsync(page, size, searchValue);
			if (res.Data != null)
			{
				return Ok(res);
			}
